          ...
          <div className="p-8 md:p-12">
            ...
            <div className="text-center mb-10">
              <motion.div
                ...
                className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 text-xs font-bold uppercase tracking-widest mb-5"
              >
                <Sparkles className="w-3 h-3" />
                Final Step
              </motion.div>
              <h1 className="text-3xl font-black text-white mb-2">Complete your profile</h1>
              <p className="text-slate-400">Join ArtMarket as an artist or a collector</p>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-7">
                  ...
                  {[
                    ...
                    return (
                      ...
                      >
                        ...
                        <div>
                          <p className={`font-bold text-sm ${isActive ? 'text-white' : 'text-slate-400'}`}>{label}</p>
                        ...
                        {isActive && (
                          ...
                          >
                            <CheckCircle className={`w-4 h-4 ${color === 'emerald' ? 'text-emerald-400' : 'text-indigo-400'}`} />
                          </motion.div>
                        )}
                      </motion.button>
                    )
              ...
              <div className="space-y-2">
                ...
                <input
                  ...
                  required
                  className="w-full px-5 py-4 bg-white/5 border border-white/10 focus:border-indigo-500/50 focus:ring-2 focus:ring-indigo-500/20 rounded-xl outline-none transition-all text-white placeholder:text-slate-600 font-medium"
                />
              </div>
                ...
                {role === 'artist' && (
                  ...
                  >
                    ...
                    <textarea
                      ...
                      rows={3}
                      className="w-full px-5 py-4 bg-white/5 border border-white/10 focus:border-indigo-500/50 focus:ring-2 focus:ring-indigo-500/20 rounded-xl outline-none transition-all text-white placeholder:text-slate-600 font-medium resize-none"
                    />
                  </motion.div>
                )}
              </AnimatePresence>
              ...
              <motion.button
                ...
                className={`w-full py-4 font-bold rounded-xl shadow-lg transition-all flex items-center justify-center gap-3 group ${
                  role === 'artist'
                    ? 'bg-indigo-600 hover:bg-indigo-500 text-white shadow-indigo-500/25 disabled:bg-indigo-800'
                    : 'bg-emerald-600 hover:bg-emerald-500 text-white shadow-emerald-500/25 disabled:bg-emerald-800'
                }`}
              >
                {loading ? (
                  <>
